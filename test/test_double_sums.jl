...
@testset "double_sums" begin
    ...
    @test isequal(
        N_modes/5, arguments(split1)[2].metadata[qc.IndexedAverageDoubleSum].sum_index.range
    )
    ...
    @test isequal(
        N_modes, arguments(split2)[2].metadata[qc.IndexedAverageDoubleSum].sum_index.range
    )
