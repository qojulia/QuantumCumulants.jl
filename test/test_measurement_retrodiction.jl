...
@testset "test_measurement_retrodiction" begin
    ...
    # dYdt(t) = dYdt_data[Int(floor(t/dt)) + 1] # TODO: why does this give a small difference?? Maybe the times do no perfectly match in the solver? Even more confusing: The tests pass with ODE-v6.103.0 and SDE-6.87.0, but not with ODE-v6.108.0 and SDE-6.94.1
    ...
    eqs_kal_1_rhs =
        eqs_det[1].rhs - s*(Ydot(t) - 2s*sqrt(Γ*η)*average(x))*sqrt(Γ*η) +
        2s*(Ydot(t) - 2s*sqrt(Γ*η)*average(x))*sqrt(Γ*η)*average(x*x) -
        2s*(Ydot(t) - 2s*sqrt(Γ*η)*average(x))*sqrt(Γ*η)*(average(x)^2)
    ...
    @test sum(abs.(sol_fw.u[end] .- sol_fw_Y.u[end])) < 1e-2 # 1e-8 works with ODE-v6.103.0 and SDE-6.87.0 ? 

    # backward equations
    eqs_back =
        meanfield_backward(ops[1:(end-1)], H, J; rates = R, efficiencies = eff, order = 2)
    ...
    # noise_Y_bw = NoiseGrid(t_W, reverse(-Y_W)) # not fully correct!
