...
@testset "indexed_scale" begin
    ...
    @test isequal(
        scale(average(∑(σ(2, 1, k)*σ(1, 2, l), k))),
        (N-1)*average(σ(2, 1, 1)*σ(1, 2, 2)) + average(σ(2, 2, 1)),
    )
    ...
    @test isequal(
        scale(average(Σ(s(1, 2, i1)*s(2, 2, i2), i1, [i2]))),
        (N-1)*average(s(1, 2, 1)*s(2, 2, 2)),
    )
    ...
    sm(i1) = (s(1, 2, i1))
    int_sum =
        Σ(sp(i1) * sm(i2) + sm(i1) * sp(i2), i1, i2) -
    ...
    int_sum_ =
        sum(σp(i1) * σm(i2) + σm(i1) * σp(i2) for i1 = 1:N_ for i2 = 1:N_) -
        sum(σp(i1) * σm(i1) + σm(i1) * σp(i1) for i1 = 1:N_)
    ...
    σ(x, y, cluster, index) =
        IndexedOperator(Transition(h, Symbol(:σ, cluster, :_), x, y, cluster), index)
    # Parameter
    ...
    Hi = sum(
        c1≠c2 && Ω(c1, c2) * ∑(σ(2, 1, c1, i(c1)), i(c1)) * ∑(σ(1, 2, c2, i(c2)), i(c2)) for
        c1 = 1:M for c2 = 1:M
    )
    H = Ha + Hd + Hi
    ...
    ops = [
        [σ(2, 2, c, k(c)) for c = 1:M];
        [σ(2, 2, c1, k(c1))*σ(2, 2, c2, l(c2)) for c1 = 1:M for c2 = 1:M]
    ]
    ...
    pn = [
        [Ω_(c1, c2) for c1 = 1:M for c2 = 1:M];
