...
@testset "average_sums" begin
    ...
    @test isequal(
        ...
        qc.insert_index(
            qc.insert_index(σ(1, 2, ind(:i))*σ(1, 2, ind(:j)), ind(:i), 2), ind(:j), 1
        ),
    )
    ...
    @test isequal(
        qc.IndexedAverageSum(g(ind(:i)), ind(:i), []), average(Σ(g(ind(:i)), ind(:i), []))
    )
    ...
    @test isequal(
        qc.eval_term(sum2_A; limits=mappingDict), evaluate(sum2_A; limits=(N_ => N_n))
    )
