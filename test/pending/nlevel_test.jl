# PENDING: port of master test/test_nlevel.jl
#
# Status: SQA-level test of the N-level operator algebra. SQA v0.5 has its
# own test suite. Notable v1 differences captured by these master
# assertions but not yet reproducible 1:1 in v1:
#   * `ha.GS` → renamed to `ha.ground_state` in v1, and returns an Int
#     (index into `ha.levels`) rather than the symbolic level identifier.
#   * `Transition(ha::NLevelSpace_with_int_levels, :σ, :g, :e)` throws
#     `ArgumentError` in v1 instead of `AssertionError`.
#   * `simplify(σ*σ') == simplify(1 - σee)` and `simplify(2*σee - 1)`
#     identities rely on the level-completeness rewrite (σ_gg + σ_ee = 1)
#     that v1 no longer applies automatically. Master had a Symbolics rule
#     that normal-ordered + closed via completeness; v1 keeps σ_gg and
#     σ_ee distinct.
#   * `QuantumCumulants.CallableTransition` is no longer exported.
#
