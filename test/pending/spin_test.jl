# PENDING: port of master test/test_spin.jl
#
# Status: this file mixes SQA-level algebra (Pauli/Spin commutation
# relations) with QC-level workflows (Dicke meanfield + ODE, collective
# spin time evolution, initial-state numeric conversion from a Ket).
#
# The QC-level meanfield + Dicke ODE assertions are already covered in
# `test/meanfield_test.jl::"meanfield: Pauli closed two-spin"` and
# `test/mtk_integration_test.jl::"Dicke model (Pauli): meanfield + complete
# + ODE"`. What is *not* yet reproducible —
#   * `Spin(::SpinSpace, ::Symbol, axis::Symbol)` — symbolic axes
#     `:x`/`:y`/`:z` are not accepted by SQA v0.5 (integer axes only).
#   * `initial_values(eqs, ψ::Ket)` for spin-coherent / Fock state
#     initialisation (CHANGELOG follow-up).
#   * `sol[op]` / `get_solution(sol, op)[end]` indexing on time series;
#     v1 uses `get_solution(sol, op, eqs)(t)` to query a single time.
#
