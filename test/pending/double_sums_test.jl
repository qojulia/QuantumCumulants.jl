# PENDING: port of master test/test_double_sums.jl
#
# Status: SQA-level (covers `DoubleSum`, `SingleSum`, `IndexedVariable`,
# `IndexedOperator`, multi-index `Σ` ordering and identities). All under
# SQA v0.5's `SecondQuantizedAlgebra` test suite.
#
...
@testset "double_sums" begin
    ...
    @test(
        isequal(
            ...
            DoubleSum(
                SingleSum(σ(2, 1, ind(:i))*σ(1, 2, ind(:j)), ind(:i), [ind(:j)]),
                ind(:j),
    ...
    @test isequal(
        ...
        simplify(Σ(-σ(2, 2, i_ind), i_ind)*4),
    )
    @test isequal(
        simplify(Σ(3*σ(2, 2, i_ind), i_ind, j_ind)),
        simplify(Σ(3*σ(2, 2, i_ind), i_ind)*4),
    )
    @test isequal(
    ...
    @test isequal(
        N_modes/5,
        arguments(split1)[2].metadata[qc.IndexedAverageDoubleSum].sum_index.range,
    )
    ...
    @test isequal(
        N_atoms/5,
        arguments(split2)[2].metadata[qc.IndexedAverageDoubleSum].innerSum.metadata[qc.IndexedAverageSum].sum_index.range,
    )

    @test isequal(
        N_modes,
        arguments(split2)[2].metadata[qc.IndexedAverageDoubleSum].sum_index.range,
    )
    ...
    Hint =
        Σ(s(2, 1, i1) * s(1, 2, i2) + s(1, 2, i1) * s(2, 1, i2), i1, i2) -
        Σ(s(2, 1, i1) * s(1, 2, i1) + s(1, 2, i1) * s(2, 1, i1), i1)
    #
