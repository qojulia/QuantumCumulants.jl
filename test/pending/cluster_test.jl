# PENDING: port of master test/test_cluster.jl
#
# Status: `ClusterSpace` was REMOVED in v1 (CHANGELOG: "use `Index` and `Σ`
# directly"). This file is kept solely as a record of the master assertions
# the cluster API used to satisfy. There is no plan to reinstate the
# `ClusterSpace`/`ClusterAon` types — the indexed equivalent is achieved
# via `Index(h, :i, N, ha)` and `Σ`.
#
