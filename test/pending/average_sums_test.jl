# PENDING: port of master test/test_average_sums.jl
#
# Status: SQA-level (covers `IndexedAverageSum`, `IndexedAverageDoubleSum`,
# `SpecialIndexedAverage`, `NumberedOperator`, `DoubleNumberedVariable`,
# `SingleNumberedVariable`, `eval_term`, `insert_index`, `create_value_map`,
# `getAvrgs`, `containsIndexedOps`, `writeNeqs`). All under SQA v0.5's
# `SecondQuantizedAlgebra` test suite; these assertions are kept here only
# to record what the master `average` algebra on indexed expressions used to
# guarantee.
#
...
@testset "average_sums" begin
    ...
    @test isequal(
        cumulant_expansion(sum_A, 2),
        qc.IndexedAverageSum(
            cumulant_expansion(average(sumterm), 2),
            ind(:i),
            [ind(:j), ind(:k)],
        ),
    )
    ...
    @test isequal(
        ...
        qc.IndexedAverageSum(average(σ(1, 2, ind(:i))), ind(:i), []),
    )
    ...
    @test ADsum1 isa SymbolicUtils.BasicSymbolic && operation(ADsum1) === +
    @test SymbolicUtils.metadata(arguments(ADsum1)[it_2])[qc.IndexedAverageDoubleSum] isa
          qc.IndexedAverageDoubleSum
    @test SymbolicUtils.metadata(arguments(ADsum1)[it_1])[qc.IndexedAverageSum] isa
          qc.IndexedAverageSum


    @test isequal(
        qc.SpecialIndexedAverage(
            average(σ(1, 2, ind(:i))),
            [(ind(:i), ind(:j))],
        )+qc.SpecialIndexedAverage(average(σ(2, 1, ind(:j))), [(ind(:i), ind(:j))]),
        qc.SpecialIndexedAverage(
            average(σ(1, 2, ind(:i))) + average(σ(2, 1, ind(:j))),
            [(ind(:i), ind(:j))],
        ),
    )
    ...
    @test SymbolicUtils.metadata(
        qc.SpecialIndexedAverage(average(σ(2, 1, ind(:i))), [(ind(:i), ind(:j))]),
    )[qc.SpecialIndexedAverage] isa qc.SpecialIndexedAverage

    @test qc.undo_average(arguments(ADsum1)[it_2]) isa qc.DoubleSum
    @test isequal(
        simplify(Σ(Σ(σ(2, 1, ind(:i))*σ(1, 2, ind(:j)), ind(:i)), ind(:j), [ind(:i)])),
        simplify(qc.undo_average(ADsum1)),
    )
    ...
    specAvrg = qc.SpecialIndexedAverage(
        average(σ(2, 1, ind(:i))*σ(1, 2, ind(:j))),
        [(ind(:i), ind(:j))],
    )
    ...
    @test isequal(
        qc.insert_index(σ(1, 2, ind(:j))*σn(1, 2, 2), ind(:j), 1),
        qc.insert_index(
            qc.insert_index(σ(1, 2, ind(:i))*σ(1, 2, ind(:j)), ind(:i), 2),
            ind(:j),
            1,
        ),
    )
    ...
    @test (
        isequal(
            qc.getAvrgs(SymbolicUtils.arguments(avrgTerm)[1]),
            average(σ(2, 1, ind(:i))*σ(1, 2, ind(:j))),
        ) || isequal(
            qc.getAvrgs(SymbolicUtils.arguments(avrgTerm)[2]),
            average(σ(2, 1, ind(:i))*σ(1, 2, ind(:j))),
        )
    )
    # SymbolicUtilsv1.4.0 argument order changed
    ...
    @test isequal(
        qc.IndexedAverageSum(g(ind(:i)), ind(:i), []),
        average(Σ(g(ind(:i)), ind(:i), [])),
    )
    @test qc.IndexedAverageSum(g(ind(:i)), ind(:i), []) isa
          SymbolicUtils.BasicSymbolic{qc.IndexedAverageSum}
    ...
    @test isequal(
        qc.eval_term(sum2_A; limits = mappingDict),
        evaluate(sum2_A; limits = (N_ => N_n)),
    )
