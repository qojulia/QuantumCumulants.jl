# PENDING: port of master test/test_fock.jl
#
# Status: this is an SQA-level operator-algebra test. SQA v0.5 has its own
# test suite covering Fock algebra (canonical commutation relations,
# Hamiltonian commutators on bare ops, scalar substitution). Keeping the
# master assertions here so a future cross-check between QC and SQA's
# Fock surface is one un-comment away.
#
# To enable: this file becomes a redundant duplicate of SQA's own
# `test/test_fock.jl`; only useful if QC needs to assert that its
# reexport surface preserves Fock semantics.
#
