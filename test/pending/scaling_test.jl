# PENDING: port of master test/test_scaling.jl
#
# Status: master's 488-line scaling test relies heavily on
# `ClusterSpace(ha, N, order)` and `ClusterAon` — both REMOVED in v1
# (CHANGELOG: "use `Index` and `Σ` directly"). The phase-invariant filter
# pattern survives in
# `test/mtk_integration_test.jl::"Single-atom laser: phase-invariant filter
# closes system"` and `"N-atom two-level laser: complete + filter_func"`,
# but the `ClusterSpace` family of assertions is not portable.
#
...
@testset "scaling" begin
    ...
    @testset "scaling cluster" begin
        ...
        H =
            Δc * a' * a +
            sum(Δ2[c] * sum(S(2, 2, c)) for c = 1:N_c) +
            sum(Δ3[c] * sum(S(3, 3, c)) for c = 1:N_c) +
            sum(Ω3[c] * (sum(S(3, 1, c)) + sum(S(1, 3, c))) for c = 1:N_c) +
            sum(g[c] * (a' * sum(S(1, 2, c)) + a * sum(S(2, 1, c))) for c = 1:N_c)
        # Collapse operators
        J = [
            a,
            [S(1, 2, c) for c = 1:N_c]...,
    ...
    @testset "scaling Two-level laser" begin
        ## Two-level laser
        M = 2
        @cnumbers N
        hf = FockSpace(:cavity)
        ...
        @test isequal(
            average(σ(:e, :e)[1] * σ(:e, :g)[2]),
            QuantumCumulants.substitute_redundants(
                avg,
                [QuantumCumulants.ClusterAon(2, 1), QuantumCumulants.ClusterAon(2, 2)],
                names,
            ),
        )
    ...
    ## Some abstract tests
    @testset "substitute_redundants" begin
        M = 4
        @cnumbers N
        hc = FockSpace(:cavity)
        hvib = FockSpace(:mode)
    ...
    ## Two clusters
    @testset "Two-level two-cluster laser" begin
        N_c = 2
        ...
        H =
            sum(Δ[c] * σ(:e, :e, c)[k] for c = 1:N_c, k = 1:M) +
            sum(g[c] * (a' * σ(:g, :e, c)[i] + a * σ(:e, :g, c)[i]) for i = 1:M, c = 1:N_c)
    ...
    @testset "6-level laser" begin
        ...
        H =
            ...
            Δ5 * sum(σ(5, 5)) +
            Δ6 * sum(σ(6, 6)) +
            Ω13 * (sum(σ(3, 1)) + sum(σ(1, 3))) +
            Ω34 * (sum(σ(3, 4)) + sum(σ(4, 3))) +
            Ω54 * (sum(σ(5, 4)) + sum(σ(4, 5))) +
            Ω64 * (sum(σ(6, 4)) + sum(σ(4, 6))) +
    ...
    # he_scale =
    #     complete(he_ops; filter_func = phase_invariant, order = order, multithread = true)
    # @test length(he_scale) == 18
    # @test isempty(find_missing(he_scale))
    # end

    ### 4th order synchronization
    # @testset "4th order 2-level laser 2 clusters synchronization" begin
    #     order = 4
    #     # Define parameters
    ...
    #         δA * sum(σA(2, 2)) +
    #         δB * sum(σB(2, 2)) +
    #         ΩA / 2 * (a'sum(σA(1, 2)) + a * sum(σA(2, 1))) +
    #         ΩB / 2 * (a'sum(σB(1, 2)) + a * sum(σB(2, 1)))
    #     # dissipative processes
