# PENDING: port of master test/test_numeric_conversion.jl
#
# Status: covers `to_numeric(op, basis; level_map=...)` and the related
# `numeric_average`, `initial_values(eqs, ψ::Ket)` machinery. `to_numeric`
# is re-exported from SQA v0.5; the v1 `initial_values(eqs, ψ::Ket)`
# overload (CHANGELOG follow-up) is not yet implemented. SQA owns the
# operator → QuantumOpticsBase conversion path; QC's contribution is the
# averaged-state initialisation.
#
