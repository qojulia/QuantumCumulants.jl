# PENDING: port of master test/test_index_basic.jl
#
# Status: SQA-level test suite for the indexed-operator machinery —
# `Index`, `IndexedOperator`, `IndexedVariable`, `DoubleIndexedVariable`,
# `SingleSum`, `DoubleSum`, `SpecialIndexedTerm`, `NumberedOperator`,
# `IndexedAverageSum`, `change_index`, `order_by_index`, `reorder`,
# `create_index_arrays`, `inorder!`, `_inconj`, `inadjoint`,
# `ismergeable`, `DoubleNumberedVariable`, `SingleNumberedVariable`,
# `get_indices`, `to_numeric`. All of these are SQA v0.5 internals (not
# part of QC's user-facing surface) and are exercised by SQA's own test
# suite.
#
# Master assertions kept here so v0.5 ↔ v1 surface drift can be audited.
#
...
@testset "index_basic" begin
    ...
    @test(
        isequal(
            SymbolicUtils.arguments(σ(1, 2, indT(:i))*a'),
            SymbolicUtils.arguments(sum1),
        )
    )
    ...
    @test isequal(
        [i_ind, j_ind],
        sort(qc.get_indices(average(σ(1, 2, i_ind)) + 3 + average(σ(2, 1, j_ind)))),
    )
