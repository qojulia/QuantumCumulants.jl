...
fws = FW[FW(let g = f; (du, u, p, t) -> (g(du, u, p, t); nothing); end) for f in fs]

driver_dyn = let fs = fs, ranges = ranges
    (du, u, p, t) -> (for (f, r) in zip(fs, ranges); f(view(du, r), u, p, t); end; nothing)
end
driver_fw = let fws = fws, ranges = ranges
    (du, u, p, t) -> (@inbounds for i in eachindex(fws); fws[i](view(du, ranges[i]), u, p, t); end; nothing)
end
driver_il = let fs = fs, ranges = ranges
    (du, u, p, t) -> (for (f, r) in zip(fs, ranges); Base.invokelatest(f, view(du, r), u, p, t); end; nothing)
end
...
for (name, drv) in ((:dyn, driver_dyn), (:fw, driver_fw), (:il, driver_il))
    ...
    println("DRIVER $name percall=$(round(tcall * 1e6, digits = 1))µs warmsolve=$(round(tsolve * 1000, digits = 1))ms")
end
