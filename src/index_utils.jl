...
get_indices(term) = SymbolicUtils.iscall(term) ? get_indices(arguments(term)) : []
...
function subst_reds_scale(term::SymbolicUtils.BasicSymbolic; kwargs...)
    ...
    for i in 1:length(avrgs)
        ind_ = findfirst(
            x -> isscaleequal(avrgs[i], x; kwargs...) && !isequal(avrgs[i], x), avrgs
        )
        if !=(ind_, nothing)
...
function isscaleequal(avr1::Average, avr2::Average; kwargs...)
...
function isscaleequal(t1, t2; kwargs...)
    if SymbolicUtils.iscall(t1) && SymbolicUtils.iscall(t2)
        args1 = arguments(t1)
        args2 = arguments(t2)
        isequal(SymbolicUtils.operation(t1), SymbolicUtils.operation(t2)) || return false
