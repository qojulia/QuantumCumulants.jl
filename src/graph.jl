...
function _derive_frontier_serial(keys, sys, ctx)
...
function _derive_frontier_parallel(keys, sys, ctx)
    out = Vector{NodeData}(undef, length(keys))
...
function _derive_frontier(keys, sys, ctx)
    if Threads.nthreads() > 1 && length(keys) > 1
        return _derive_frontier_parallel(keys, sys, ctx)
    end
...
function _closure(
        g::MomentGraph, derive_frontier; filter = _alltrue, get_adjoints::Bool = true,
        foldable = _alltrue, max_iter::Int = 100_000,
    )
    ctx = g.ctx
    ...
    while !isempty(frontier)
        ...
        for i in eachindex(frontier)
            ...
            if processed >= max_iter
                pending = length(frontier) - i + 1 + length(newkeys)
                error(
                    "closure did not close the hierarchy within $max_iter iterations " *
                        "($pending moments still pending); the system may not close.",
                )
            end
            processed += 1
            nd = nodes[frontier[i]]
            for leaf in _drift_leaves(nd)
                ...
                # Discovery, de-duplication and ordering stay serial. Only the independent
                # `derive` calls for the completed frontier are parallelized.
                push!(seen, k)
                push!(newkeys, k)
                if kc != k && !(kc in seen)
                    if get_adjoints
