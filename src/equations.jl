...
mutable struct ScaledHeisenbergEquation{LHS,RHS,H,J,R,A,N,W,T} <: AbstractEquation{LHS,RHS}
    lhs::Vector{LHS}
    rhs::Vector{RHS}
    hamiltonian::H
    jumps::J
...
Base.hash(eq::ScaledHeisenbergEquation, h::UInt) = hash(eq.reference_terms, hash(eq.was_scaled, hash(eq.names, hash(eq.scale_aons, hash(eq.rates, hash(eq.jumps, hash(eq.hamiltonian, hash(eq.rhs, hash(eq.lhs, h)))))))))
Base.isequal(eq1::ScaledHeisenbergEquation,eq2::ScaledHeisenbergEquation) = isequal(hash(eq1), hash(eq2))

Base.getindex(de::ScaledHeisenbergEquation, i::Int) = ScaledHeisenbergEquation([de.lhs[i]],[de.rhs[i]],de.hamiltonian,de.jumps,de.rates,de.scale_aons,de.names,de.was_scaled[i],de.reference_terms)
Base.getindex(de::ScaledHeisenbergEquation, i) = ScaledHeisenbergEquation(de.lhs[i],de.rhs[i],de.hamiltonian,de.jumps,de.rates,de.scale_aons,de.names,de.was_scaled[i],de.reference_terms)
...
function substitute(de::ScaledHeisenbergEquation,dict)
    lhs = [substitute(l, dict) for l in de.lhs]
    rhs = [substitute(r, dict) for r in de.rhs]
    return ScaledHeisenbergEquation(lhs,rhs,de.hamiltonian,de.jumps,de.rates,de.scale_aons,de.names,de.was_scaled,de.reference_terms)
end
