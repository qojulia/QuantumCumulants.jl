...
systems on a multi-threaded Julia process; pass `true` to select it explicitly. The
...
function KernelBackend(; parallel = false)
    parallel === :auto ||
        parallel isa Bool ||
...
KernelRHS(kernel::MomentKernel) = KernelRHS(kernel, nothing)
...
function _build_rhs(
    ...
    if !dojac
        kernel = MomentKernel(ir; parallel)
        parameters = KernelParameters(ir, kernel.pattern, values)
        return KernelRHS(kernel), parameters
    end
    ...
    kernel = MomentKernel(ir_ext; parallel)
    jacobian = JacKernel(ir_ext, jir)
    parameters = KernelParameters(ir_ext, kernel.pattern, values)
    return KernelRHS(kernel, jacobian), parameters
end
...
function SciMLBase.ODEFunction(
    ...
    rhs, _ = _build_rhs(eqs, ps, backend; jac)
...
function SciMLBase.ODEProblem(
    ...
    rhs, p = _build_rhs(eqs, ps, backend; jac)
    f = _ode_function(rhs)
    return SciMLBase.ODEProblem(f, _u0_vector(eqs, u0), tspan, p; kwargs...)
end
...
Refresh the numeric coefficient data in a direct problem's parameter payload. The structural
lowering and monomial table are reused, so repeated solves and parameter sweeps do not rederive
equations. The same operation can be applied directly to `prob.p` before `remake(prob; p=...)`.
"""
function update_parameters!(kp::KernelParameters, pdict)
    fresh = kernel_pdict(kp.params, pdict; strict = false)
    merge!(kp.values, fresh)
    cvals = kp.evalcoeffs(kp.values)
    copyto!(kp.coeffs, cvals)
    write_nzval!(kp)
    return kp
end

function update_parameters!(prob::SciMLBase.ODEProblem, pdict)
    prob.p isa KernelParameters || throw(
        ArgumentError(
            "update_parameters! is only available for direct KernelBackend problems.",
        ),
    )
    update_parameters!(prob.p, pdict)
    return prob
end

function update_parameters!(::SciMLBase.ODEFunction, pdict)
    throw(
        ArgumentError(
            "update_parameters! needs the ODEProblem so it can update prob.p; " *
                "call update_parameters!(prob, pdict) or update_parameters!(prob.p, pdict).",
        ),
    )
end
