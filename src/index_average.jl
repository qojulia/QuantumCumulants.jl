...
function eval_term(
    ...
    if meta.sum_index.range in keys(limits)
        ...
        if meta.sum_index.range isa BasicSymbolic{<:Complex{<:Real}}
            ...
            if SymbolicUtils.iscall(meta.sum_index.range)
                ...
                if SymbolicUtils.operation(meta.sum_index.range) === *
...
function eval_term(sum::BasicSymbolic{IndexedAverageDoubleSum}; kwargs...)
    ...
    return eval_term(
        IndexedAverageDoubleSum(
            eval_term(meta.innerSum; kwargs...), meta.sum_index, meta.non_equal_indices
        );
        kwargs...,
    )
end
...
function check_arr(lhs, arr)
    ...
    for i in 1:length(hilbert(args[1]).spaces)
...
function getAvrgs(sum::BasicSymbolic{SpecialIndexedAverage})
    getAvrgs(SymbolicUtils.metadata(sum)[SpecialIndexedAverage].term)
end
function getAvrgs(sum::BasicSymbolic{IndexedAverageSum})
    getAvrgs(SymbolicUtils.metadata(sum)[IndexedAverageSum].term)
end
function getAvrgs(Dsum::BasicSymbolic{IndexedAverageDoubleSum})
...
function simplifyMultiplication(term::BasicSymbolic{<:CNumber})
    if SymbolicUtils.iscall(term) && operation(term) === *
        args = arguments(term)
        ind = findfirst(x -> (SymbolicUtils.iscall(x) && operation(x) === +), args)
