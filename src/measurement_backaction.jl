...
function translate_W_to_Y(eqs::NoiseEquations; simplify = true, mix_choice = maximum)
    ...
    J = eqs_copy.jumps
    Jd = eqs_copy.jumps_dagger
    rates = eqs_copy.rates
...
function _meanfield_backaction(
    ...
    function calculate_term(i)
        ...
        rhs[i] = rhs_ + rhs_diss # + rhs_dY
        rhs_noise[i] = _master_noise(a[i], J, Jdagger, efficiencies .* rates)
    end
    ...
    rhs_noise = map(undo_average, rhs_noise_avg) # this is not correct, see _master_noise (but also not used) TODO
...
# TODO: this function is never used and it does not consider the noise equations
