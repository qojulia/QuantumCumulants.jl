function scale(he, scale_aons, N; kwargs...)
    M = length(scale_aons)
    rhs = []
    names = get_names(he)
    for i=1:length(he)
        aon_lhs = acts_on(he.lhs[i])
        ...
        rhs_ = qsimplify(he.rhs[i]; rewriter=rw)
        rhs_ = substitute_redundants(rhs_, scale_aons, names)

        push!(rhs, rhs_)
    end
    return HeisenbergEquation(he.lhs, rhs, he.hamiltonian, he.jumps, he.rates)
end

function get_names(he)
    H = he.hamiltonian
    J = he.jumps
    ops_lhs = [l.arguments[1] for l in he.lhs]
    ops = get_operators(H)
    for j ∈ J
...
function should_scale(x, scale_aons, aon_lhs, was_scaled)
    _in(x, was_scaled) && return false # x was already scaled
    aon = acts_on(x)
    ...
    should_scale_ && push!(was_scaled, x)
