...
abstract type AbstractAverage end # <: Number ?
abstract type BasicAverage <: AbstractAverage end

# Keep track of averages when converting to SymbolicUtils
const AVERAGES_TO_SYMS = Dict{BasicAverage,SymbolicUtils.Sym}()
const SYMS_TO_AVERAGES = Dict{SymbolicUtils.Sym,BasicAverage}()

struct Average{OP} <: BasicAverage
    operator::OP
    function Average{OP}(operator::OP) where OP<:AbstractOperator
        avg = new(operator)
        if !haskey(AVERAGES_TO_SYMS, avg)
            sym = SymbolicUtils.Sym{Number}(gensym(:average))
            AVERAGES_TO_SYMS[avg] = sym
            SYMS_TO_AVERAGES[sym] = avg
        end
        return avg
    end
end
Average(operator::OP) where OP = Average{OP}(operator)
Base.:(==)(a1::Average,a2::Average) = (a1.operator==a2.operator)

struct AverageTerm{F,ARGS} <: AbstractAverage
    f::F
    arguments::ARGS
end
Base.:(==)(t1::AverageTerm,t2::AverageTerm) = (t1.f===t2.f && t1.arguments==t2.arguments)

for f = [:+,:-,:*,:/]
    @eval Base.$f(a::AbstractAverage,b::AbstractAverage) = AverageTerm($f, [a,b])
end
Base.:*(a::AbstractAverage,b::Number) = AverageTerm(*, [a,b])
Base.:*(a::Number,b::AbstractAverage) = AverageTerm(*, [a,b])
Base.:*(a::AbstractAverage,b::SymbolicUtils.Symbolic{<:Number}) = AverageTerm(*, [a,b])
Base.:*(a::SymbolicUtils.Symbolic{<:Number},b::AbstractAverage) = AverageTerm(*, [a,b])
Base.:^(a::AbstractAverage,b) = AverageTerm(^, [a,b])

# TODO: Variadic methods

Base.conj(a::Average) = Average(adjoint(a.operator))
Base.conj(t::AverageTerm) = AverageTerm(t.f, conj.(t.arguments))

average(op::BasicOperator) = Average(op)
function average(op::OperatorTerm)
    if op.f ∈ [+,-]
        return op.f(average.(op.arguments)...)
    elseif op.f ∈ [*,⊗]
        ...
        if isempty(cs)
            ...
            return prod(cs)*Average(op.f(ops...))
        end
    else
        return Average(op)
    end
...
_to_symbolic(a::Average) = AVERAGES_TO_SYMS[a]
_to_symbolic(a::AverageTerm) = SymbolicUtils.Term{Number}(a.f, _to_symbolic.(a.arguments))
function _to_average(s::SymbolicUtils.Sym)
...
_to_average(t::SymbolicUtils.Term) = AverageTerm(t.f, _to_average.(t.arguments))
_to_average(x::Number) = x

function simplify_averages(avg::AbstractAverage;kwargs...)
    s = _to_symbolic(avg)
    s_ = SymbolicUtils.simplify(s;kwargs...)
    return _to_average(s_)
end
...
# function get_order(a::OperatorTerm{F}) where F<:Union{typeof(*),typeof(⊗)}
